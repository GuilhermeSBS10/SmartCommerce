...
public class UserService {
    ...
    public UserResponse updateProfile(User currentUser, UpdateProfileRequest request) {
        ...
        User updated = usuarioRepository.update(new User(
                currentUser.id(),
                request.nome().trim(),
    ...
    public UserResponse updateSecurity(User currentUser, UpdateSecurityRequest request) {
        if (!currentUser.senha().equals(request.senhaAtual())) {
        ...
        User updated = usuarioRepository.update(new User(
                currentUser.id(),
                currentUser.nome(),
                request.novoEmail() != null && !request.novoEmail().isBlank()
                        ? request.novoEmail().trim().toLowerCase(Locale.ROOT)
                        : currentUser.email(),
                request.novaSenha() != null && !request.novaSenha().isBlank()
                        ? request.novaSenha()
                        : currentUser.senha(),
